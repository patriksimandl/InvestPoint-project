...
export function EmailInput({ email, setEmail, emailCredentials,resetKey }: EmailInputProps) {
  ...
  const showEmailError = !isFocused && !emailCredentials && haveBeenClicked;
  const showMandatoryError = showEmailError && email === '';
  ...
  return (
    <>
      <div className="input-container relative">
        <input
          className={`input-primary ${showEmailError ? 'input-primary-error' : ''}`}
          onFocus={() => { setIsFocused(true) }}
          onBlur={() => { setIsFocused(false) }}
          onChange={updateEmail}
          value={email}
          placeholder=""
          onClick={() => { setHaveBeenClicked(true) }}
        />
      ...
      {showEmailError?
        ...
        <div className='w-full rounded-[8px] flex '>
          ...
          <div className='text-red-700 text-[15px]'>{showMandatoryError ? 'Manditory' : 'Invalid email'}</div>
