...
export function NameInput({name,setNameValidation}: NameInputProps) {
  ...
  const showMandatoryError = !inputFocus && nameValue.trim() === '' && haveBeenClicked;

  return (
    <div className='flex flex-col'>

      <div  className="input-container">
        <input className={`input-primary ${showMandatoryError ? 'input-primary-error' : ''}`} onFocus={()=>{setInputFocus(true)}} onBlur={()=>{setInputFocus(false)}} onChange={(event) => { setNameValidation(event, name); setNameValue(event.target.value) }} value={nameValue} onClick={()=>{setHaveBeenClicked(true)}}
          placeholder="" />
